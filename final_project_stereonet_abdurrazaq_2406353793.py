# -*- coding: utf-8 -*-
"""Final Project Stereonet_Abdurrazaq_2406353793

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1jO2Oy4XbuZejrKjuIl8bGWSHz3Mepi3G
"""
...
# numpy buat bantuin kita melakukan hitung-hitung vektor, trigonometri, array — wajib ada
import numpy as np

# matplotlib buat bantuin gambar semua per plot plot annya (stereonet, rose, beachball, dll)
import matplotlib.pyplot as plt
import matplotlib.patches as mpatches
from matplotlib.lines import Line2D

# Counter dari collections itu bisa bantuin kita buat ngitung frekuensi tiap tipe Riedel
from collections import Counter

# mplstereonet nambahin proyeksi stereonet ke dalam matplotlib
# tanpa ini kita ga akan bisa bikin ax dengan projection='stereonet'
import mplstereonet

# obspy itu ya sebenarnya cuman library seismologi, kita cuma butuh satu fungsi: beach() buat beachball
# dibungkus try/except karena obspy agak berat laptopnya takut ga kuat, kalau ga keinstall app tetap jalan
try:
...
# Kita disini melakukan konfigurasi halaman
# ini harus paling atas pokoknya sebelum st yang lain, kalau ngga bakal error
...
# CSS custom buat kita polish tampilannya jadi lebih oke
# IBM Plex Sans buat teks biasa, IBM Plex Mono buat angka & judul — keliatan lebih technical, ini ibaratnya kita setting kaya word
st.markdown("""
<style>
...
#FUNGSI MATEMATIKA & GEOMETRI, bagian cukup rumit heheh

def mean_angle(angles):
    # rata-rata sudut yang bener, bukan pakai sum/n kaya biasa
    # nah masalahnya itu: rata-rata 10° dan 350° harusnya 0° bukan 180°
    # Akhirnya saya dapetin solusi: uraikan ke sin & cos dulu, baru arctan2
    r = np.radians(angles)
...
def angular_diff(a, b):
    # pakai fungsi modulo 360 dulu biar ga negative, trus intinya ambil yang terkecil
    d = abs(a - b) % 360
    return min(d, 360 - d)
...
def plane_to_vec(strike, dip):
    ...
    # vektor ini sebenarnya adalahbagian  pole = garis tegak lurus bidang
    # rumusnya saya dapet dari geometri bola: sin(dip)*sin(strike) dst
    s = np.radians(strike)
    d = np.radians(dip)
    v = np.array([
    ...
    if v[2] < 0:
        v = -v   # pastiin kalau proyeksi selalu di lower hemisphere
    return v / np.linalg.norm(v)   # jangan lupa normalisasi jadi unit vektor


def normalize(v):
    # jadiin vektor jadi panjang 1 (bagian dari unit vektor)
    # terus kaya ada pengecekan kecil biar ga dibagi 0
    n = np.linalg.norm(v)
    return v / n if n > 1e-9 else v


def vec_to_trend_plunge(v):
    # balik lagi pokoknya dari vektor 3D ke trend/plunge yang bisa dibaca manusia
...
def classify_fold_fleuty(plunge, interlimb_angle=None):
    ...
    # Subvertical     : 80–90°
    if plunge < 10:
        plunge_class = "Recumbent / subhorizontal"
    elif plunge < 30:
        plunge_class = "Gently plunging"
    elif plunge < 60:
        plunge_class = "Moderately plunging"
    elif plunge < 80:
        plunge_class = "Steeply plunging"
    else:
        plunge_class = "Subvertical / upright"

    # terus jangan lupa juga tambahin Klasifikasi tightness dari interlimb angle (sudut antar limb):
    ...
    elif interlimb_angle > 120:
        tight_class = "Gentle fold (>120°)"
    elif interlimb_angle > 70:
        tight_class = "Open fold (70°–120°)"
    elif interlimb_angle > 30:
        tight_class = "Close fold (30°–70°)"
    elif interlimb_angle > 5:
        tight_class = "Tight fold (5°–30°)"
    else:
        tight_class = "Isoclinal fold (<5°)"
...
def calc_interlimb_angle(fold_left, fold_right):
    ...
    # Pasti bertanya kenapa dari pole? karena sudut antar pole = sudut antar bidang
    # (suplemen dari sudut antar normal = sudut antar limb itu sendiri sebenarnya)
    def mv(sets):
        vecs = [plane_to_vec(s['strike'], s['dip']) for s in sets]
        return normalize(np.mean(vecs, axis=0))
    mL = mv(fold_left)
    mR = mv(fold_right)
    # terus kita musti bikin dot product dua unit vektor = cos(sudut antar keduanya)
    cos_a = np.clip(np.dot(mL, mR), -1, 1)
    angle_between_poles = np.degrees(np.arccos(cos_a))
    # interlimb angle = 180° - sudut antar pole (karena seperti yang kita tau kalau pole tegak lurus bidang)
    interlimb = 180.0 - angle_between_poles
    return float(interlimb)

def classify_fault_by_rake(rake):
    # Disini kita pake klasifikasi sesar dari rake — ini yang paling akurat
    # rake = sudut arah slip di permukaan bidang sesar (-180 sampai +180°)
    # Referensi yang kita: Aki & Richards (2002), Twiss & Moores (2007)
...
def classify_fault_by_dip(dips):
    # fallback klasifikasi kalau pitch semua = 0 (belum diisi manual, dia akan memaksa isi)
    # pakai dip rata-rata sebagai proxy kalau misalnya kurang akurat tapi cukuplah untuk estimasi
    # ada tambahan catatan: sesar strike-slip bisa dip-nya curam (~90°), makanya ini bisa aja salah sebenarnya
    # makanya mendingan lebih baik isi pitch/rake secara manual untuk hasil yang bener
    arr = np.array(dips)
    n   = len(arr)
...
def estimate_rake(mean_dip, sense):
    # Kita harus melakukan estimasi rake otomatis kalau pengguna ga ngisi pitch
    # ini cuma fallback kasar ajasih sebenarnya biar hasilnya bisa salah seperti kasus di atas
    # (dip 85° bisa aja bahkan jadi strike-slip kalau rake-nya kecil)
    # isi pitch manual ajasih kalau mau klasifikasi yang bener
    if mean_dip > 60:   return -90
    elif mean_dip < 30: return 0
    else:               return 30 if 'Dextral' in sense else -30

def classify_joint(dip_angle):
    # Lalu kita masuk ke bagian klasifikasi kekar berdasarkan sudut dip-nya
    # sistemnya bikin sederhana aja: vertikal, subvertikal, miring, subhorizontal
...
def classify_joint_system(strikes):
    # Kita cek dulu pokoknya apakah kekar-kekarnya punya orientasi yang sistematis (>1 set arah)
    # nah gimanasih caranya: hitung selisih tiap pasang, kalau ada yang ~90° berarti ada 2 set
    # ini cara paling sederhana sih, tapi di beberapa kasus untuk data lebih banyak idealnya pakai clustering aja
    if len(strikes) < 2:
...
# Di baris ini kita akan fokus ke baris plot

def plot_fault_stereonet(fault_sets):
    # KIta pake stereonet utama buat data sesar
    # nah nantinya di setiap set di-plot great circle (bidang) + pole (titik tegak lurus)
    fig, ax = plt.subplots(
        figsize=(5.5, 5.5),
        subplot_kw=dict(projection='stereonet')  # ini yang nantinya bakal bikin jadi lingkaran stereonet
    )
    ...
    for i, fs in enumerate(fault_sets):
        col = COLORS[i % len(COLORS)]   # kita juga perlu untuk cycling warna biar ga error kalau data > 8

        # ax.plane() gambar great circle ini nantinya akan representasi bidang penuh di stereonet
        ax.plane(fs['strike'], fs['dip'], color=col, linewidth=1.5, alpha=0.7)

        # ax.pole() gambar titik pole = biar kaya jadi lebih enak dibaca kalau data banyak
        ax.pole(fs['strike'], fs['dip'], color=col, marker='o', markersize=7)

        lbl = "Set %d  %d°/%d°" % (i + 1, fs['strike'], fs['dip'])
        if fs.get('pitch', 0):
            lbl += "  pitch %d°" % fs['pitch']   # tambahin pitch ke label kalau diisi nantinya
    ...
    # karena label defaultnya sering ketimpa frame atau posisinya aneh, makanya kita adjustable
    ...
    # legend ditaruh aja di bawah dengan bbox_to_anchor supaya ga nutupin stereonet
    # ncol=2 biar ga terlalu panjang ke bawah kalau banyak set, jadi ga kegeser intinya
...
def plot_rose(strikes):
    # rose diagram = histogram di polar axes, buat nantinya kita bisa lihat orientasi dominan strike
    fig = plt.figure(figsize=(4.5, 4.5))
    ax  = fig.add_subplot(111, polar=True)   # polar=True yang bikin outputnya jadi melingkar nanti
    fig.patch.set_facecolor('#FAFAF8')
    ax.set_facecolor('#FAFAF8')

    # data strike didobel aja dengan +180° karena seperti yang kita tau kan satu bidang punya dua arah berlawanan
    # kalau ga didobel rose-nya bakalan asimetris padahal kan secara geologi harusnya simetris dia
    ...
    # utara tuh kan posisinya di atas, jadi secara ga langsung kan searah jarum jam makanya konvensi standar kompas geologi
    ax.set_theta_zero_location('N')
    ax.set_theta_direction(-1)

    ax.bar(bins[:-1], n, width=2 * np.pi / 36,
           bottom=0, color='#185FA5', edgecolor='white', alpha=0.8, linewidth=0.5)
    ax.set_yticks([])   # hapus aja kalau ada label jari-jari, menurut saya sih kurang informatif di rose
    ax.set_title("Rose Diagram (Strike)", fontsize=11, pad=14)
    plt.tight_layout()
    return fig

def plot_stress_stereonet(sigma1, sigma2, sigma3):
    # Kita definisikan dulu stereonet buat nampilin posisi sumbu tegasan utama (σ1, σ2, σ3)
    ...
    for trend, col, lbl, ls, mk in [
        ...
        # strike bidang tegasan itu kan trendnya kan 90° karena tegak lurus sama sumbu-nya
        ax.plane((trend - 90) % 360, 90, color=col, linewidth=2, linestyle=ls)
        # plungeitu =0 karena masih asumsi sumbu horizontal (penyederhanaan)
...
def plot_beachball(strike, dip, rake):
    # kitta disini adjustable buaat beachball = focal mechanism, biar nantinya bisa nampilin pola slip sesar
    # nah ini pembentuk beachball kuadran gelap = compressional (daerah tekan, P-axis)
    # kuadran terang tu artinya = dilatational (daerah tarik ekstensi berarti, T-axis)
    ...
    if OBSPY_OK:
        try:
            # beach() dari obspy itu sebenarnya adalah cara paling akurat buat kita ngegambar beachball
            # input: [strike, dip, rake] dari bidang sesar utama yang ada di baris sebelum sebelumnya
            b = beach(
                [strike, dip, rake],
                xy=(0, 0), width=1.9,
        ...
        # kalau obspy ga ada atau ga keliatan, ya mau ga mau gambar manual sederhana
        # ini cuma approximasi visual aja, bukan proyeksi yang akurat banget
...
def plot_fold_stereonet(fold_left, fold_right):
    # Kita definisikan lagi stereonet lipatan: plot kedua limb + hitung sumbu lipatan
    fig, ax = plt.subplots(
        figsize=(5.5, 5.5),
        subplot_kw=dict(projection='stereonet')
    )
    ...
    # plot semua data yan bisa dilihat dari limb kiri (biru solid) dan kanan (oranye putus-putus)
    for fs in fold_left:
    ...
    def mean_vec(sets):
        # kalkuklasi rata-rata vektor = dengan ngitung mean di ruang 3D Cartesian
        # bisa buat lebih stabil juga dari rata-rata sudut langsung,
        # terutama kalau data yang kita punya nyebrang 0°/360°
        vecs = [plane_to_vec(s['strike'], s['dip']) for s in sets]
        return normalize(np.mean(vecs, axis=0))

    mL = mean_vec(fold_left)    # mean pole limb kiri di def
    mR = mean_vec(fold_right)   # mean pole limb kanan di def juga

    # fold axis = biar kita nanti bisa cross product dua mean pole
    # kenapa sih haruscross product? karena cross product itu bisa bantu dari dua normal bidang
    # vektor yang sejajar dengan garis perpotongan kedua bidang yang kita gunakan
    # oleh karena itu definisi sumbu lipatan secara geometri 3 dimensii
    fv = normalize(np.cross(mL, mR))
    if fv[2] < 0:
        fv = -fv   # nah terus baris ini tuh buat flip ke lower hemisphere biar konsisten
    if np.isnan(fv[0]):
        # kalau dua limb paralel, maka cross product nya itu 0 fallback ke rata-rata
    ...
    # segitiga nantinya buat mean pole beda dari lingkaran data individual
    ax.line(pL, tL, marker='^', color='#185FA5', markersize=13, zorder=5)
    ax.line(pR, tR, marker='^', color='#D85A30', markersize=13, zorder=5)

    # bintang hijau buat fold axis ukuran gede biar bisa gampang ketemu
    ...
    # legend manual yang eksplisit biar ga ada simbol simbol misterius nantinya
    handles = [
...
def plot_joint_stereonet(joint_sets):
    # stereonet kekar nah sebenarnya agak mirip sesar tapi biasanya lebih banyak data
    # kekar = rekahan tanpa pergerakan, jadi makanya kaya ga  tuh yang namanya rake/pitch
    fig, ax = plt.subplots(
        figsize=(5.5, 5.5),
        subplot_kw=dict(projection='stereonet')
    )
    ...
    for i, js in enumerate(joint_sets):
        ...
        ax.pole(js['strike'],  js['dip'], color=col, marker='D',
                markersize=6)   # marker kaya diamond (D) page nya biar beda dari sesar (circle)
...
def plot_joint_rose(strikes):
    # rose diagram khusus kekar itu akan sama persis dengan rose sesar
    # tapi emang sebaiknya dipisah sih fungsinya biar bisa dikustomisasi warnanya berbeda
    fig = plt.figure(figsize=(4.5, 4.5))
    ...
    # warna hijau tuh bisa buat teal buat bedain dari rose sesar yang biru
    ax.bar(bins[:-1], n, width=2 * np.pi / 36,
...
def plot_joint_contour(joint_sets):
    # nah jadi disini kita mendefinisikam contour plot (density plot) buat lihat clustering pole kekar
    # ini tuh akan berguna banget kalau misalnaya dataset kita banyak dan mau lihat ada berapa set kekar
    ...
    # pokoknys kits plot semua pole dulu sebagai titik
    for i, js in enumerate(joint_sets):
        col = COLORS[i % len(COLORS)]
        ax.pole(js['strike'], js['dip'], color=col, marker='D', markersize=5, alpha=0.7)

    # dia bakal auto density contour kalau data >= 3 (minimal buat interpolasi), kaya kalsbeek
    if len(joint_sets) >= 3:
        try:
            # cmap 'Greens' disini itu dipilih biar konsisten dengan tema warna kekar
            ...
            pass   # ya kalau nanti misalnya gagal (misal data terlalu sedikit), skip aja juga gapapa
...
# SESSION STATE ini tuh bakal berguna banget buat nyimpen data input biar ga ilang tiap rerun
# ya karena tuh streamlit rerun script dari atas tiap ada perubahan input
# session_state adalah cara ampuh lah intinya buat simpan data yang persist antar rerun

if 'fault_sets' not in st.session_state:
    # kita bisa liat kalau contoh data sesar konjugat (dua kelompok strike berlawanan ~180°)
...
if 'joint_sets' not in st.session_state:
    # contoh data kekar tiga set dengan orientasi berbeda, ini biar ga ke reset aja sih state sessionnya
...
#Disini kita bhuat header page nya dengan membedakan antara bagian sesar, lipatan, dan kekar
st.title("🪨 Stereonet Geologi Struktur")
st.caption("Proyeksi stereonet interaktif — Sesar, Lipatan & Kekar")
st.divider()

# pilihan halamannya disini saya pakai radio horizontal di atas
mode = st.radio(
...
#Nah disini kita udah mulai masuk membuat landing page dari sesar
if mode.startswith("⚡"):
    ...
    with col_input:
        ...
        for i, s in enumerate(fs):
            ...
            with cc:
                # pitch = isi rake manual, opsional
                # kalau diisi 0 berarti akan selalu dihitung otomatis
        ...
        # hapus dilakukan di luar loop biar index yang kita punya ga kacau
...
# disini kita melakukan pengecekan apakah ada pitch yang ga diisi (> 0)
        # bukan rata-rata karena kalau 5 set pitch=0 dan 1 set pitch=10, jadi ga seimbang
        # makanya kita buat rata-ratanya tetap kecil dan salah masuk ke fallback dip
        pitched = [s.get('pitch', 0) for s in fs if s.get('pitch', 0) > 0]
        pitch_diisi = len(pitched) > 0

        if pitch_diisi:
            # nah biar ga jompang pakai rata-rata dari pitch yang diisi aja (yang 0 di-skip biar hasilnya sesuai)
            mean_pitch = float(np.mean(pitched))
            rake = int(mean_pitch)
        else:
            # kalau semisalnya belum ada pitch sama sekali, kita bisa melakukan estimasi dari dip + shear sense
            mean_pitch = 0.0
            rake = estimate_rake(float(np.mean(dips)), shear_sense)

        # Klasifikasi sesar pakai rake kalau pitch sudah diisi, fallback dip kalau misalnya belum
        # tanda * di hasil berarti estimasi dari dip, bukan rake berarti kurang akurat
        if pitch_diisi:
            fault_type = classify_fault_by_rake(rake)
            rake_source = "dari pitch manual"
        else:
            # fallback: klasifikasi dari dip — bisa aja salah untuk strike-slip dip curam
            # misalnya: dip 85 + rake 10 = strike-slip, tapi dip doang kelihatan normal fault
            fault_type = classify_fault_by_dip(list(dips))
            rake_source = "estimasi dari dip — isi Pitch untuk hasil akurat"

        # Calculate stress axes once for both plotting and metrics
        ...
        with tab1:
            ...
            plt.close(fig)   # plt.close() ini penting banget buat free memory, kalau ga lama2 berat, laptop ga kuat
        ...
        with tab4:
            # Ini harus ada setidaknya satu bidang sesar untuk beachball
            if len(fs) > 0:
                main_fault = fs[0]
                # kalau pitch diisi, pakai itu. Kalau tidak, pakai rake yang diestimasi
                bb_rake = main_fault.get('pitch', 0) if main_fault.get('pitch', 0) > 0 else rake
                fig = plot_beachball(main_fault['strike'], main_fault['dip'], bb_rake)
                st.pyplot(fig, use_container_width=True)
                plt.close(fig)
            else:
                st.info("Masukkan setidaknya satu data sesar untuk melihat beachball.")

# disini saya melakukan analisis Riedel shear, dengan cara kelompokin tiap strike ke tipe geser
# shear zone = bin dengan frekuensi strike terbanyak
hist = np.zeros(18)   # ni misal ya 18 bin × 20° = 360°
for s in strikes:
...
# nah ini akan jadi bahaya kalau pitch belum diisi disini saya minta kasih tau kalau belum diisi
    # nih ya karena kalau dip 85° tanpa pitch diklasifikasikan normal fault, padahal bisa strike-slip
if not pitch_diisi:
...
#nah terakhir kita buat landing page dari kekar
    ...
    with col_input:
        ...
        # kekar tuh kan ga punya arah slip, jadi cukup strike + dip aja
    ...
    # Disini saya akan klasifikasi tiap kekar satu per satu
    ...
    # sistem kekar itu musti berdasarkan orientasi antar-bidang
    joint_system = classify_joint_system(list(strikes_j))

    # bisa di hitung rata-rata strike & dip kekar
...
#terus jangan lupa juga bikin footer
st.divider()
st.caption(
